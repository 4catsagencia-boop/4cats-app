...
const casesEs: CaseStudy[] = [
  ...
  },
  {
    ...
    verified: false,
    url: "gruashardy.cl",
    industry: "Izaje & Transporte Industrial",
    industryColor: "#F59E0B",
    title: "Hardy Grúas · La Araucanía",
    subtitle: "De WordPress genérico a plataforma de servicios industriales 24/7",
    challenge: "Hardy Grúas opera con un sitio WordPress estático con un único canal de contacto: teléfono. Sin formulario de cotización, sin descripción técnica de capacidades (toneladas, alcance, certificaciones), sin galería con contexto y con textos genéricos que no diferencian su servicio de rescate y izaje industrial en la región.",
...
const casesEn: CaseStudy[] = [
  ...
  },
  {
    ...
    verified: false,
    url: "gruashardy.cl",
    industry: "Crane & Industrial Transport",
    industryColor: "#F59E0B",
    title: "Hardy Grúas · La Araucanía",
    subtitle: "From generic WordPress to a 24/7 industrial services platform",
    challenge: "Hardy Grúas runs a static WordPress site with a single contact channel: phone. No quote form, no technical specs (tonnage, reach, certifications), no gallery with context — and generic copy that doesn't differentiate their industrial rescue and crane service in the region.",
...
function CaseCard({ c, lang }: { c: CaseStudy; lang: "es" | "en" }) {
  return (
    ...
    >
      ...
      {/* In-progress banner */}
      {c.inProgress && (
        <div className="bg-[#6366F1] px-6 py-2 flex items-center justify-between">
          <div className="flex items-center gap-2">
      ...
      <div className="px-8 py-6 border-b border-[#F4F4F5] flex-1">
        <p className="text-xs font-semibold text-[#A1A1AA] uppercase tracking-wider mb-3">
          {c.inProgress
            ? (lang === "es" ? "Métricas objetivo" : "Target metrics")
      ...
      {/* ROI callout */}
      {c.inProgress ? (
        <div className="px-8 py-6 bg-gradient-to-r from-[#EEF2FF] to-[#E0E7FF]">
          <p className="text-xs font-semibold text-[#6366F1] uppercase tracking-wider mb-1">
            {lang === "es" ? "Resultados proyectados" : "Projected results"}
          </p>
          <p className="text-lg font-bold text-[#18181B] mb-1">
            {lang === "es" ? "Pendiente al cierre del proyecto" : "Pending at project close"}
          </p>
